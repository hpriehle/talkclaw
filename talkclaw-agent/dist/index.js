...
app.get("/health", (_req, res) => {
    res.json({ status: "ok", service: "talkclaw-agent" });
});
// ── TalkClaw webhook ────────────────────────────────────────────────────────
...
app.post("/webhook/talkclaw", (req, res) => {
    ...
    handleMessage(content, `talkclaw-${sessionId}`, systemPrompt, {
        replyFn: async (text) => {
            console.log(`[talkclaw] Sending reply to session ${sessionId} (${text.length} chars)`);
            try {
                const response = await fetch(`${TALKCLAW_SERVER_URL}/api/v1/sessions/${sessionId}/messages`, {
                    method: "POST",
                    headers: {
        ...
        deltaFn: async (delta, _msgId) => {
            ...
            try {
                await fetch(`${TALKCLAW_SERVER_URL}/api/v1/sessions/${sessionId}/delta`, {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
                        Authorization: `Bearer ${TALKCLAW_API_TOKEN}`,
...
if (TELEGRAM_BOT_TOKEN) {
    ...
    bot.on("message", (msg) => {
        ...
        handleMessage(msg.text, `telegram-${chatId}`, systemPrompt, {
            replyFn: async (text) => {
