...
extension View {
    ...
    @ViewBuilder
    func liquidGlass(in shape: Capsule) -> some View {
        if #available(iOS 26.0, *) {
            self.glassEffect(in: shape)
        } else {
            self
    ...
    /// Interactive liquid glass — scales on press, shimmers, illuminates on touch
    @ViewBuilder
    func interactiveGlass(in shape: Capsule = Capsule()) -> some View {
        if #available(iOS 26.0, *) {
    ...
    /// Interactive glass with circle shape
    @ViewBuilder
    func interactiveGlass(in shape: Circle) -> some View {
        if #available(iOS 26.0, *) {
            self.glassEffect(.regular.interactive(), in: shape)
        } else {
            self.background(.ultraThinMaterial, in: shape)
    ...
    /// Interactive glass with rounded rectangle shape
    @ViewBuilder
    func interactiveGlass(in shape: RoundedRectangle) -> some View {
        if #available(iOS 26.0, *) {
            self.glassEffect(.regular.interactive(), in: shape)
        } else {
            self.background(.ultraThinMaterial, in: shape)
